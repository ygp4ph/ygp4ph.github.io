...
def clean_content(html_content):
    ...
    # Supprime les liens vers d'autres pages Notion (bookmarks cassés)
    content = re.sub(r'<figure[^>]*class="[^"]*link-to-page[^"]*"[^>]*>.*?</figure>', '', content, flags=re.DOTALL)
    
    # Convertit les bookmarks Notion en liens simples
    def convert_bookmark(match):
        ...
        if href_match:
            ...
            return f'<p><a href="{href}" target="_blank" rel="noopener">{title}</a></p>'
        return ''
